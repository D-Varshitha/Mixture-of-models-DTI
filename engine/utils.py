...
def save_model(model, args, epoch, result_df=None):
    ...
    # BUG FIX: respect --root argument instead of always using cwd
    root_dir = getattr(args, "root", None) or os.getcwd()
    exp_dir  = os.path.join(root_dir, "dataset", safe_data, safe_model,
                            safe_exp, safe_custom)
    ...
    if result_df is not None:
        result_df.to_csv(
            os.path.join(exp_dir, f"epoch{epoch}_results.csv"), index=False
        )
